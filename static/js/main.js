...
function readCallLogs(){
  ...
  configs['showBlocked'] = ($('#showBlocked').is(":checked")) ? true : false
  configs['withRecording'] = ($('#withRecording').is(":checked")) ? true : false
  configs['dateFrom'] = $("#fromdatepicker").val() + "T00:00:00.000Z"
  configs['dateTo'] = $("#todatepicker").val() + "T23:59:59.999Z"
  configs['perPage'] = 1000

  var url = "/readlog?access=" + $('#access_level').val();
  url += "&params=" + JSON.stringify(configs)
  var posting = $.get( url );
  posting.done(function( response ) {
    ...
    if (res.hasOwnProperty('calllog_error')){
      ...
      callLogsData = new CallLogsData(res)
      drawGraphs()
    }
  });
